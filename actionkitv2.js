...
forms.loadContext = function() {
    ...
    /*
    Context preloading can be used by a script like this first thing in the
    document head:
      actionkitContext=[];
      var s=document.createElement('script');
      s.src='/context/{{page.name}}'+(location.search||'?')+'&callback=actionkitContext.push&r='+Math.random();
      s.type='text/javascript';
      document.head.appendChild(s);
    ...but don't do that yet because it's not tested and ready for prime time.
    */
    ...
    if ( preload ) {
        if ( preload.length )
            ...
            return window.actionkitContext = {push: actionkit.forms.onContextLoaded};
    }
...
forms.prefill = function(overwrite) {
    ...
    // fire onchange, for in-field labels etc.
    $(ak.form).find(':input').change();
}
...
forms.required = function() {
    ...
    // them
    if ( !ak.form ) {
        required = ['email', 'country', 'zip', 'first_name', 'last_name'];
    }
    else {
...
forms.initPage = function() {
    ...
    if ( $.browser.msie && ak.args.debug ) {
        ...
        document.write("<script type='text/javascript' src='http://getfirebug.com/releases/lite/latest/firebug-lite.js'></scr" + "ipt>");
    }
};
...
forms.getLabel = function(input) {
    ...
    return $('label[for=' + input.id + '], label[for=id_' + input.name + ']');
}
